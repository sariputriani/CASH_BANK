                    ...
                    if(response.success && response.data) {
                        ...
                        // ============================
                            ...
                            if (response.data.kategori_id) {
                                $('#kategori')
                                    .val(response.data.kategori_id)
                                    .trigger('change');
                            }

                            // ============================
                            // SET SUB KRITERIA
                            // (beri delay agar dropdown ter-load)
                            // ============================
                            setTimeout(function () {
                                if (response.data.sub_kriteria_id) {
                                    $('#sub_kriteria')
                                        .val(response.data.sub_kriteria_id)
                                        .trigger('change');
                                }
                            }, 300);

                            // ============================
                            // SET ITEM SUB KRITERIA
                            // ============================
                            setTimeout(function () {
                                if (response.data.item_sub_kriteria_id) {
                                    $('#item_sub_kriteria')
                                        .val(response.data.item_sub_kriteria_id)
                                        .trigger('change');
                                }
                            }, 600);
