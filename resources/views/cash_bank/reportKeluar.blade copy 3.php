            ...
            <div class="card rounded-3 border-0 shadow-sm" id="printArea">
                <div class="card-body table-responsive">
                                ...
                                @forelse($rekapVA as $kategori => $subs)
                                    ...
                                    @foreach($subs as $sub => $items)
                                        ...
                                        @foreach($items as $item)
                                            <tr>
                                                @if(!$firstKategori)
                                                    <td rowspan="{{ $rowspanKategori }}">{{ $kategori }}</td>
                                                    @php $firstKategori = true; @endphp
                                                @endif

                                                @if(!$firstSub)
                                                    <td rowspan="{{ $rowspanSub }}">{{ $sub }}</td>
                    ...
                    @if(!request('rekapanVA'))
                        <div class="alert alert-info mt-3">
                            <small>Filter: {{ implode(' | ', $filterInfo) }}</small>
                        </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            @if($data->count() > 0 && $showSaldoAkhir)
                                <tfoot class="table-secondary">
                                    <tr>
                                        <th colspan="{{ 7 + ($countActiveFilters >= 2 ? 1 : 0) + ($showDebet ? 1 : 0) }}" class="text-end">TOTAL SALDO AKHIR:</th>
                                    ...
                                    <tr>
                                        <th colspan="{{ 7 + ($countActiveFilters >= 2 ? 1 : 0) + ($showDebet ? 1 : 0) }}" class="text-end">TOTAL KREDIT:</th>
                                    ...
                                    <tr>
                                        <th colspan="{{ 7 + ($countActiveFilters >= 2 ? 1 : 0) + ($showDebet ? 1 : 0) }}" class="text-end">TOTAL DEBET:</th>
                            ...
                            @else{
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end fw-bold">@currency($data->sum('kredit'))</td>
                                    </tr>
                                </tfoot>
                            }
                            @endif
