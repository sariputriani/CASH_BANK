...
if ($rekapanVA === 'va') {
    ...
    foreach ($rekapBanks as $bank) {
        ...
        // Hitung saldo akhir secara kumulatif untuk bank ini
        $saldoAkhir = 0;
        
        // Query untuk mendapatkan transaksi terakhir per bank dengan saldo running
        if ($showDebet) {
            // Gabungkan bank masuk dan keluar untuk bank ini
            $transaksiBank = DB::table('bank_masuk')
                ->select('tanggal', 'debet', DB::raw('0 as kredit'), 'id_bank_masuk as id')
                ->where('id_bank_tujuan', $bank->id_bank_tujuan)
                ->where(function($q) use ($applyFilter) {
                    $applyFilter($q, 'bank_masuk');
                })
                ->unionAll(
                    DB::table('bank_keluars')
                        ->select('tanggal', DB::raw('0 as debet'), 'kredit', 'id_bank_keluar as id')
                        ->where('id_bank_tujuan', $bank->id_bank_tujuan)
                        ->where(function($q) use ($applyFilter) {
                            $applyFilter($q, 'bank_keluars');
                        })
            ...
            $saldo = 0;
            foreach ($transaksiBank as $t) {
                $saldo += ($t->debet ?? 0) - ($t->kredit ?? 0);
            }
            $saldoAkhir = $saldo;
        } else {
            // Hanya ada transaksi keluar
            $saldoAkhir = -$kredit; // Negatif karena hanya pengeluaran
        }
        ...
        if ($debet > 0 || $kredit > 0) {
            $rekapVA[] = [
                ...
                'debet' => $debet,
                'kredit' => $kredit,
                'saldo_va' => $saldoAkhir,  // ← Saldo kumulatif terakhir
                'saldo_sap' => '-',
                'selisih' => $saldoAkhir,
                'keterangan' => '-'
            ];
