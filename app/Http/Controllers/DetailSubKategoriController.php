...
use App\Exports\excelDetaillKategori;
...
class DetailSubKategoriController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = DB::query()
            ...
            // TIDAK ADA FILTER ITEM - Ini yang penting!
            ->orderBy('trx.tanggal', 'asc')
            ->get();
        ...
        return view('cash_bank.detail.detailSub', compact(
            'data',
            'kategori',
            'sub',
            'tahun',
    ...
    public function export_excel(Request $request){
        ...
        return Excel::download(new excelDetaillKategori($request), 'detail_'.$sub.'_'.$tahun.'.xlsx');
    }
    ...
    public function view_pdf(Request $request){
        ...
        return view('cash_bank.exportPDF.subPdf', compact(
            'data',
            'kategori',
            'sub',
            'tahun',
 ...
 // {
    ...
    //     // Cek apakah ini untuk semua item (total sub) atau item spesifik
    //     $isAllItems = ($item === 'ALL' || $item === 'all' || empty($item));

    //     try {
    //         // Query untuk Bank Keluar
    //         $queryKeluar = bankKeluar::select(
    //                 'bank_keluars.*',
    ...
    //         // Filter item: jika bukan ALL, filter berdasarkan item spesifik
    //         if (!$isAllItems) {
    //             $queryKeluar->where('item_sub_kriteria.nama_item_sub_kriteria', 'LIKE', "%{$item}%");
    //         }

    //         // Query untuk Bank Masuk (jika diperlukan)
    //         $queryMasuk = bankMasuk::select(
    //                 'bank_masuk.*',
    //                 'kategori_kriteria.nama_kriteria',
    //                 'sub_kriteria.nama_sub_kriteria',
    //                 'item_sub_kriteria.nama_item_sub_kriteria',
    ...
    //         if (!$isAllItems) {
    //             $queryMasuk->where('item_sub_kriteria.nama_item_sub_kriteria', 'LIKE', "%{$item}%");
    //         }

    //         // Gabungkan hasil dari bank_keluar dan bank_masuk
    //         $dataKeluar = $queryKeluar->get();
    //         $dataMasuk = $queryMasuk->get();
            
    //         // Merge dan sort berdasarkan tanggal
    //         $data = $dataKeluar->concat($dataMasuk)->sortBy('tanggal')->values();

    //         // Debugging
    //         \Log::info('Detail Sub Kategori Query', [
    //             'kategori' => $kategori,
    //             'sub' => $sub,
    //             'item' => $item,
    //             'isAllItems' => $isAllItems,
    //             'tahun' => $tahun,
    //             'keluar_count' => $dataKeluar->count(),
    //             'masuk_count' => $dataMasuk->count(),
    //             'total_count' => $data->count(),
    //             'total_kredit' => $dataKeluar->sum('kredit'),
    //             'total_debet' => $dataMasuk->sum('debet')
    //         ]);

    //     } catch (\Exception $e) {
    //         \Log::error('Error Query Detail Sub Kategori: ' . $e->getMessage());
    ...
    //     // Hitung total kredit dan debet
    //     $totalKredit = $data->sum('kredit');
    //     $totalDebet = $data->sum('debet');

    //     // Judul yang berbeda untuk ALL items vs item spesifik
    //     $displayItem = $isAllItems ? "Semua Item dalam {$sub}" : $item;

    //     return view('cash_bank.detail.detailSub', [
    //     'data' => $data,
    //     'kategori' => $kategori,
    //     'sub' => $sub,
    //     'item' => $displayItem,
